...
void Usage::simulateUsage(int billCalendarId){
/*
...
vector<Usage> currentUsage = loadUsage("data/usage.txt", billCalendarId);
...
cout << prevBillCalendarId <<endl;
vector<Usage> prevUsage = loadUsage("data/usage.txt", prevBillCalendarId);

if (!currentUsage.empty()) {
    cout << "Usage already simulated for Bill Calendar " << billCalendarId << "\n";
    return;
}
if (prevUsage.empty()) {
    cout << "Error No Usage ";
